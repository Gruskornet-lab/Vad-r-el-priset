...
def fetch_prices(zone: str, date: datetime) -> list[dict]:
    ...
    for entry in raw_data:
        ...
        # Parse the timestamp and convert to CET (UTC+1) before extracting hour.
        # The API returns timestamps with timezone info, e.g. "2026-04-01T13:00:00+01:00".
        # On GitHub's UTC servers, parsing without explicit CET conversion can cause
        # the same CET hour to appear twice (once as UTC, once as CET), or hours
        # to be off by 1. We always normalize to CET to get the correct Swedish hour.
        time_start = entry["time_start"]
        dt = datetime.fromisoformat(time_start)
        cet = timezone(timedelta(hours=1))
        hour = dt.astimezone(cet).hour
