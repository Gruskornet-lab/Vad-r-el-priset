...
STOCKHOLM     = ZoneInfo("Europe/Stockholm")  # Handles CET/CEST automatically
...
def run_evening_summary():
    ...
    tomorrow_str = (datetime.now(tz=STOCKHOLM) + timedelta(days=1)).strftime("%Y-%m-%d")
...
def run_hourly_check():
    ...
    """
    now_cet = datetime.now(tz=STOCKHOLM)
